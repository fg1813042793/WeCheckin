...
注册表拒绝重复键和未知处理器；流程任务使用 `business_type=scheduled_task`、`business_key=<run_id>`，相同运行重试返回已有实例而不创建第二条。
...
流程配置包含定义 ID、版本策略、固定版本、发起人和表单 JSON。扩展工作流应用服务，使业务唯一键冲突时查询并返回已有实例；不改变普通后台发起行为。
