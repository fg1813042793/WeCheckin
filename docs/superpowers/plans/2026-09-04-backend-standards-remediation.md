...
- [ ] **Step 4：让主服务实际使用 Server/CORS 配置**
...
- [ ] **Step 3：删除 resolver 内部 `context.Background()`**
...
- [ ] **Step 3：Setup handler 使用服务层 Context 版本**
...
- Modify: `backend/cmd/taskd/main.go`
- Modify: `backend/cmd/taskd/main_test.go`
- Create: `backend/migrations/20260904110000_create_notification_outbox.sql`
...
- [ ] **Step 3：适配通用通知 outbox**
  ...
  - `20260904110000_create_notification_outbox.sql` 同时幂等写入 code=`system.notification-outbox-dispatch`、minute cron、enabled=1 的系统任务，首次 `next_run_at` 设为迁移执行时刻。
