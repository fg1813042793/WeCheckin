...
- 表单支持只读计算组件，可按标签结果或独立字段展示；公式使用字段编码引用普通数字/金额字段，支持常量、括号及 `+`、`-`、`*`、`/`。明细字段通过 `SUM`、`AVG`、`MIN`、`MAX`、`COUNT` 聚合，并允许在同一明细行内先组合不同数字列再聚合，例如 `SUM([items.quantity] * [items.price])`。计算字段不能引用其他计算字段，也不能作为明细列；客户端实时预览，后端在草稿、发起、任务处理和表单修改时重新计算并保存权威结果。
