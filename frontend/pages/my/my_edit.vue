<template>
  <view class="container">
    <view class="form">
      <view class="form-item avatar-item" @click="chooseAvatar">
        <text class="form-label">头像</text>
        <view class="avatar-wrap">
          <image v-if="formPic" :src="formPic" mode="aspectFill" class="avatar"></image>
          <text v-else class="avatar-text">{{ (formName || '?').charAt(0) }}</text>
        </view>
      </view>
      <view class="form-item">
        <text class="form-label">昵称</text>
        <input class="form-input" v-model="formName" placeholder="请输入昵称" placeholder-class="placeholder" />
      </view>
      <view class="form-item">
        <text class="form-label">手机号</text>
        <input class="form-input" v-model="formMobile" placeholder="请输入手机号" type="number" maxlength="11" placeholder-class="placeholder" />
      </view>
    </view>

    <view class="form" v-if="deptName || topDeptName">
      <view class="form-group-header">部门信息</view>
      <view class="form-item" v-if="topDeptName">
        <text class="form-label">顶级部门</text>
        <text class="form-value readonly">{{ topDeptName }}</text>
      </view>
      <view class="form-item" v-if="deptName">
        <text class="form-label">部门</text>
        <text class="form-value readonly">{{ deptName }}</text>
      </view>
    </view>

    <view class="form" v-if="formFields.length > 0">
      <view class="form-group-header">扩展信息</view>
      <view class="form-item" v-for="(f, fi) in formFields" :key="fi">
        <text class="form-label">{{ f.label }}<text v-if="f.required" class="required">*</text></text>
        <input v-if="f.type === '文本' || !f.type" class="form-input" v-model="f.value" :placeholder="'请输入' + f.label" />
        <input v-else-if="f.type === '数字'" class="form-input" type="number" v-model="f.value" :placeholder="'请输入' + f.label" />
        <textarea v-else-if="f.type === '多行文本'" class="form-textarea" v-model="f.value" :placeholder="'请输入' + f.label" />
        <picker v-else-if="f.type === '选择'" class="field-picker" :range="f.optionsArr || []" @change="(e) => { f.value = (f.optionsArr || [])[e.detail.value] }">
          <view class="form-input picker-text" :class="{ placeholder: !f.value }">{{ f.value || '请选择' + f.label }}</view>
        </picker>
        <picker v-else-if="f.type === '日期'" class="field-picker" mode="date" :value="datePart(f.value)" @change="setFieldValue(f, $event)">
          <view class="form-input picker-text" :class="{ placeholder: !f.value }">{{ f.value || '请选择日期' }}</view>
        </picker>
        <picker v-else-if="f.type === '时间'" class="field-picker" mode="time" :value="timePart(f.value)" @change="setFieldValue(f, $event)">
          <view class="form-input picker-text" :class="{ placeholder: !f.value }">{{ f.value || '请选择时间' }}</view>
        </picker>
        <view v-else-if="f.type === '日期时间'" class="datetime-field">
          <picker class="datetime-picker" mode="date" :value="datePart(f.value)" @change="setDateTimeDate(f, $event)">
            <view class="form-input picker-text datetime-input" :class="{ placeholder: !datePart(f.value) }">{{ datePart(f.value) || '选择日期' }}</view>
          </picker>
          <picker class="datetime-picker" mode="time" :value="timePart(f.value)" @change="setDateTimeTime(f, $event)">
            <view class="form-input picker-text datetime-input" :class="{ placeholder: !timePart(f.value) }">{{ timePart(f.value) || '选择时间' }}</view>
          </picker>
        </view>
      </view>
    </view>

    <view class="submit-btn" @click="submit">保存</view>
  </view>
</template>

<script>
import { passportApi, userFormFields } from '../../api/index'
import { getClientUserId, getClientUserInfo, setClientUserInfo } from '../../utils/auth'
import { uploadFile } from '../../utils/upload'

export default {
  data() {
    return {
      formName: '',
      formMobile: '',
      formPic: '',
      formFields: [],
      deptName: '',
      topDeptName: ''
    }
  },

  onLoad() {
    this.loadDetail()
  },

  onPullDownRefresh() {
    this.loadDetail()
  },

  methods: {
    getUserId() {
      return getClientUserId()
    },

    async loadDetail() {
      try {
        const uid = this.getUserId()
        if (!uid) {
          uni.stopPullDownRefresh()
          uni.redirectTo({ url: '/pages/my/my_reg' })
          return
        }
        const [userRes, configRes] = await Promise.all([
          passportApi.getMyDetail({ user_id: uid }),
          userFormFields()
        ])
        const data = userRes.data || {}
        if (data.id) {
          this.formName = data.name || ''
          this.formMobile = data.mobile || ''
          this.formPic = data.avatar || ''
          this.deptName = data.deptName || ''
          this.topDeptName = data.topDeptName || ''
        } else {
          uni.stopPullDownRefresh()
          uni.redirectTo({ url: '/pages/my/my_reg' })
          return
        }

        const list = Array.isArray(configRes.data) ? configRes.data : []
        const userForms = this.parseForms(data.forms || data.formList || [])
        this.formFields = list.map(f => ({
          label: f.label,
          type: f.type || '文本',
          required: !!f.required,
          options: f.options || '',
          optionsArr: f.options ? f.options.split(',').map(s => s.trim()) : [],
          value: userForms[f.label] || ''
        }))
      } catch (e) {
        uni.stopPullDownRefresh()
        uni.redirectTo({ url: '/pages/my/my_reg' })
      }
      uni.stopPullDownRefresh()
    },

    parseForms(raw) {
      if (!raw) return {}
      try {
        const arr = typeof raw === 'string' ? JSON.parse(raw) : raw
        if (Array.isArray(arr)) {
          const map = {}
          for (const item of arr) {
            map[item.label] = item.value
          }
          return map
        }
        return {}
      } catch (e) {
        return {}
      }
    },

    datePart(value) {
      const match = String(value || '').match(/\d{4}-\d{2}-\d{2}/)
      return match ? match[0] : ''
    },

    timePart(value) {
      const match = String(value || '').match(/\d{2}:\d{2}/)
      return match ? match[0] : ''
    },

    setFieldValue(field, e) {
      field.value = e.detail.value || ''
    },

    setDateTimeDate(field, e) {
      field.value = this.joinDateTime(e.detail.value || '', this.timePart(field.value))
    },

    setDateTimeTime(field, e) {
      field.value = this.joinDateTime(this.datePart(field.value), e.detail.value || '')
    },

    joinDateTime(date, time) {
      return [date, time].filter(Boolean).join(' ')
    },

    chooseAvatar() {
      uni.chooseImage({
        count: 1,
        sizeType: ['compressed'],
        sourceType: ['album', 'camera'],
        success: async (res) => {
          const tempFilePaths = res.tempFilePaths
          if (tempFilePaths.length > 0) {
            uni.showLoading({ title: '上传中...' })
            try {
              const uploaded = await uploadFile(tempFilePaths[0], { scope: 'client' })
              this.formPic = uploaded.url
              uni.showToast({ title: '上传成功', icon: 'success' })
            } catch (e) {
              uni.showToast({ title: e.message || '上传失败', icon: 'none' })
            } finally {
              uni.hideLoading()
            }
          }
        }
      })
    },

    async submit() {
      if (!this.formName) {
        uni.showToast({ title: '请输入昵称', icon: 'none' })
        return
      }

      const forms = this.formFields.filter(f => f.value).map(f => ({ label: f.label, value: f.value }))
      const formsStr = forms.length > 0 ? JSON.stringify(forms) : ''

      try {
        uni.showLoading({ title: '保存中...', mask: true })
        const uid = this.getUserId()
        const res = await passportApi.editBase({
          name: this.formName,
          mobile: this.formMobile,
          pic: this.formPic,
          user_id: uid,
          forms: formsStr
        })
        uni.hideLoading()
        if (res.code === 0) {
          const userInfo = getClientUserInfo() || {}
          userInfo.name = this.formName
          userInfo.avatar = this.formPic
          setClientUserInfo(userInfo)
          uni.showToast({ title: '保存成功', icon: 'success' })
          setTimeout(() => {
            uni.navigateBack()
          }, 1500)
        } else {
          uni.showToast({ title: res.msg || '保存失败', icon: 'none' })
        }
      } catch (e) {
        uni.hideLoading()
        uni.showToast({ title: '保存失败', icon: 'none' })
      }
    }
  },
}
</script>

<style scoped>
.container {
  min-height: 100vh;
  background-color: #f5f5f5;
}

.form {
  margin: 20rpx;
  background-color: #fff;
  border-radius: 16rpx;
  overflow: hidden;
}

.form-group-header {
  padding: 24rpx 30rpx 8rpx;
  font-size: 28rpx;
  font-weight: bold;
  color: #333;
  border-bottom: 1rpx solid #f5f5f5;
}

.form-item {
  display: flex;
  align-items: center;
  padding: 30rpx;
  border-bottom: 1rpx solid #f5f5f5;
}

.form-item:last-child {
  border-bottom: none;
}

.form-label {
  font-size: 30rpx;
  color: #333;
  width: 120rpx;
  flex-shrink: 0;
  margin-right: 20rpx;
}

.form-input {
  flex: 1;
  font-size: 30rpx;
  color: #333;
}

.form-value {
  flex: 1;
  font-size: 30rpx;
  color: #666;
}

.readonly {
  color: #999;
}

.placeholder {
  color: #ccc;
}

.picker-text {
  min-height: 44rpx;
  line-height: 44rpx;
}

.field-picker {
  flex: 1;
  min-width: 0;
}

.datetime-field {
  flex: 1;
  min-width: 0;
  display: flex;
  gap: 16rpx;
}

.datetime-picker {
  flex: 1;
  min-width: 0;
}

.datetime-input {
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}

.required {
  color: #fb454c;
}

.form-textarea {
  flex: 1;
  min-height: 80rpx;
  font-size: 30rpx;
  color: #333;
}

.avatar-item {
  justify-content: space-between;
}

.avatar-wrap {
  display: flex;
  align-items: center;
}

.avatar {
  width: 100rpx;
  height: 100rpx;
  border-radius: 50%;
  margin-right: 16rpx;
}
.avatar-text {
  width: 100rpx;
  height: 100rpx;
  border-radius: 50%;
  background-color: #fb454c;
  color: #fff;
  font-size: 40rpx;
  display: flex;
  align-items: center;
  justify-content: center;
  margin-right: 16rpx;
  flex-shrink: 0;
}

.avatar-arrow {
  font-size: 28rpx;
  color: #ccc;
}

.submit-btn {
  margin: 60rpx 20rpx;
  background-color: #fb454c;
  color: #fff;
  text-align: center;
  padding: 28rpx;
  border-radius: 16rpx;
  font-size: 32rpx;
  font-weight: bold;
}
</style>
