  ...
  <view class="container">
    ...
    <view class="list" v-if="list.length > 0">
      <view class="card" v-for="(item, i) in list" :key="i" @click="goDetail(item)">
        ...
        <view class="card-score" v-if="item.myScore !== undefined">
          <text class="score-label">我的成绩：</text>
          <text class="score-val">{{ item.myScore }}</text>
        </view>
...
.card-score { margin-bottom: 8rpx; }
.score-label { font-size: 26rpx; color: #666; }
.score-val { font-size: 32rpx; color: #fb454c; font-weight: bold; }
