...
func TestRoleHandlerAcceptsAdminPermissionKeys(t *testing.T) {
	...
	for _, snippet := range []string{
		...
		"AddWithAssignmentsContext(ctx, admin.ID, name, remark, c.ClientIP(), sort, dataScope, allowAdminLogin, adminPermissionKeys, adminAPIPermissionKeys",
		"EditWithAssignmentsContext(ctx, admin.ID, uint(id), name, remark, c.ClientIP(), sort, status, dataScope, allowAdminLogin, adminPermissionKeys, adminAPIPermissionKeys",
	} {
