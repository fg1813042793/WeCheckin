...
func GetEnrollUserRank(enrollID string) ([]model.EnrollUser, error) {
	return GetEnrollUserRankContext(context.Background(), enrollID)
}

func GetEnrollUserRankContext(ctx context.Context, enrollID string) ([]model.EnrollUser, error) {
	db, cancel := database.WithContext(ctx)
	defer cancel()
	var list []model.EnrollUser
	err := db.Where("`enroll_user_enroll_id` = ?", enrollID).
...
func GetMyEnrollUserList(userID string) ([]model.EnrollUser, error) {
	return GetMyEnrollUserListContext(context.Background(), userID)
}

func GetMyEnrollUserListContext(ctx context.Context, userID string) ([]model.EnrollUser, error) {
	db, cancel := database.WithContext(ctx)
	defer cancel()
	var list []model.EnrollUser
	err := db.Where("`enroll_user_mini_openid` = ?", userID).Order("`enroll_user_add_time` DESC").Find(&list).Error
	if err != nil {
		return nil, err
	}
	...
	for i := range list {
		...
		if err := db.Where("`id` = ?", list[i].EnrollID).First(&enroll).Error; err == nil {
