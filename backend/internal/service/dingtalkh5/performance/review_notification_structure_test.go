...
func TestReviewNextHandlerAccountUsesCurrentStatus(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := reviewNextHandlerAccount(review); got != tt.want {
				t.Fatalf("reviewNextHandlerAccount() = %q, want %q", got, tt.want)
			}
		})
	}
}
...
func TestReviewTransitionActionsScheduleNextHandlerNotification(t *testing.T) {
	...
	for _, fn := range []string{
		...
		if !strings.Contains(body, "notifyReviewTransitionAsync(ctx, transitionedReview, user, dingtalkH5NotifyEventFlowMoved)") {
	...
	if !strings.Contains(returnBody, "notifyReviewTransitionAsync(ctx, transitionedReview, user, dingtalkH5NotifyEventFlowMoved)") {
...
func TestCreateReviewSchedulesEmployeeNotification(t *testing.T) {
	...
	if !strings.Contains(body, "notifyReviewTransitionAsync(ctx, review, user, dingtalkH5NotifyEventFlowMoved)") {
...
func TestDingTalkH5NotificationSourceHasSwitchAndBindingLookup(t *testing.T) {
	...
	notifyBody := functionBody(text, "func notifyReviewTransitionAsync")
	if strings.Contains(notifyBody, "DingTalkH5NotificationEnabledContext(ctx)") {
		t.Fatalf("notification scheduling should not use one global switch before resolving the enterprise app config")
	}
	sendBody := functionBody(text, "func sendReviewTransitionNotificationContext")
	for _, want := range []string{
		"nextAccount := reviewNextHandlerAccount(review)",
		"resolveDingTalkH5NotifyRecipientContext(ctx, review.EmployeeAccount, nextAccount)",
	} {
