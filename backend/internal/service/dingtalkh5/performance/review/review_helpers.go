...
func findVisibleReview(ctx context.Context, user *model.DingTalkH5PerfUser, reviewNo string) (*model.DingTalkH5PerfReview, error) {
	...
	var err error
	query, err = reviewscope.ApplyVisibilityScopeContext(ctx, db, query, user, reviewscope.ScopeSummary)
	if err != nil {
		return nil, err
	}
...
func mutateReview(ctx context.Context, user *model.DingTalkH5PerfUser, reviewNo string, action func(*gorm.DB, *model.DingTalkH5PerfReview) error) (*ReviewDTO, error) {
	...
	err := db.Transaction(func(tx *gorm.DB) error {
		...
		visible, err := reviewscope.InDataScopeContext(ctx, tx, user, output)
		if err != nil {
			return err
		}
		if !visible {
...
func latestHistoriesByReviewIDs(ctx context.Context, reviewIDs []uint) (map[uint]model.DingTalkH5PerfHistory, error) {
	result := make(map[uint]model.DingTalkH5PerfHistory, len(reviewIDs))
	if len(reviewIDs) == 0 {
		return result, nil
	}
	db, cancel := database.WithContext(ctx)
	defer cancel()
	subQuery := db.Model(&model.DingTalkH5PerfHistory{}).
		Select("MAX(id)").
		Where("review_id IN ?", reviewIDs).
		Group("review_id")
	var histories []model.DingTalkH5PerfHistory
	if err := db.Where("id IN (?)", subQuery).Find(&histories).Error; err != nil {
		return nil, err
	}
	for _, history := range histories {
		result[history.ReviewID] = history
	}
	return result, nil
}
