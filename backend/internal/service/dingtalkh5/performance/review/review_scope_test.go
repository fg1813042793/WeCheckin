...
func TestListReviewsSkipsHydrationQueriesWhenPageIsEmpty(t *testing.T) {
	src, err := os.ReadFile("reviews.go")
	if err != nil {
		t.Fatalf("read reviews.go: %v", err)
	}
	body := functionBody(string(src), "func listReviewsContext")
	emptyGuard := strings.Index(body, "if len(reviews) == 0 {")
	if emptyGuard < 0 {
		t.Fatalf("empty review pages should return before participant/history/template hydration")
	}
	for _, snippet := range []string{
		"usersByAccounts(ctx",
		"historiesByReviewIDs(ctx",
		"latestHistoriesByReviewIDs(ctx",
		"loadReviewValueTemplatesContext(ctx)",
	} {
		if idx := strings.Index(body, snippet); idx >= 0 && idx < emptyGuard {
			t.Fatalf("empty review page guard must run before %s", snippet)
		}
	}
}
