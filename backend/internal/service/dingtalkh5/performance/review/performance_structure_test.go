...
func TestReviewListSupportsPaginationAndBatchHistories(t *testing.T) {
	...
	for _, snippet := range []string{
		...
		"historiesByReviewIDs(ctx, reviewIDs)",
	} {
...
func TestDingTalkH5WorkbenchUsesAggregateCounts(t *testing.T) {
	src, err := os.ReadFile("../../bootstrap/service.go")
	if err != nil {
		t.Fatalf("read dingtalk h5 bootstrap service: %v", err)
	}
	text := string(src)
	for _, snippet := range []string{
...
func TestBootstrapIncludesPermissionVersionWithoutFullData(t *testing.T) {
	bootstrapSrc, err := os.ReadFile("../../bootstrap/service.go")
	if err != nil {
		t.Fatalf("read dingtalk h5 bootstrap service: %v", err)
	}
	combined := string(bootstrapSrc)
	for _, snippet := range []string{
		"PermissionVersion",
