...
func TestMain(m *testing.M) {
	if err := os.Chdir("../../../internal/service"); err != nil {
		panic(err)
	}
	os.Exit(m.Run())
}
...
func TestDingTalkH5ServiceUsesBusinessSubpackages(t *testing.T) {
	for _, file := range []string{
		filepath.Join("dingtalkh5", "account", "service.go"),
		filepath.Join("dingtalkh5", "auth", "service.go"),
		filepath.Join("dingtalkh5", "bootstrap", "service.go"),
		filepath.Join("dingtalkh5", "config", "app_config.go"),
		filepath.Join("dingtalkh5", "config", "corp_config.go"),
		filepath.Join("dingtalkh5", "config", "dingtalk_oapi.go"),
		filepath.Join("dingtalkh5", "performance", "review", "reviews.go"),
		filepath.Join("dingtalkh5", "performance", "review", "review_query.go"),
		filepath.Join("dingtalkh5", "performance", "review", "review_create.go"),
		filepath.Join("dingtalkh5", "performance", "review", "scope", "scope.go"),
		filepath.Join("dingtalkh5", "performance", "review", "service.go"),
		filepath.Join("dingtalkh5", "performance", "template", "service.go"),
		filepath.Join("dingtalkh5", "performance", "user", "service.go"),
	} {
		if _, err := os.Stat(file); err != nil {
