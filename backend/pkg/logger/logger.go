...
var (
	...
	sqlLogger = log.New(os.Stdout, "\r\n", log.LstdFlags)
)

func SQLWriter() *log.Logger {
	return sqlLogger
}
...
func Init(logDir, level string, maxAge int, compress bool) error {
	...
	sqlLog := &lumberjack.Logger{
		Filename:  filepath.Join(logDir, fmt.Sprintf("sql-%s.log", time.Now().Format("2006-01-02"))),
		MaxAge:    maxAge,
		Compress:  compress,
		LocalTime: true,
	}
	...
	sqlLogger = log.New(io.MultiWriter(os.Stdout, sqlLog), "\r\n", log.LstdFlags)
	log.SetOutput(io.MultiWriter(os.Stdout, errorLog))
	log.SetFlags(log.LstdFlags | log.Lshortfile)

	go rotateDaily(logDir, accessLog, errorLog, sqlLog)

	return nil
}

func rotateDaily(logDir string, accessLog, errorLog, sqlLog *lumberjack.Logger) {
	for {
		now := time.Now()
		...
		sqlLog.Filename = filepath.Join(logDir, fmt.Sprintf("sql-%s.log", time.Now().Format("2006-01-02")))
		accessLog.Rotate()
		errorLog.Rotate()
		sqlLog.Rotate()
	}
