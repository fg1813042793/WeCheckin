...
- 请求链路只检查必需 schema，结构缺失时返回可操作的迁移提示，不在 handler/service 中建表或加列；统一在维护窗口运行 `backend/init.sh`。
...
- 创建表时要对应现有数据的字符集和排序规则。

## 8. 配置、日志与外部边界

- 配置结构在 `internal/config` 集中管理，YAML 与 `WECHECKIN_` 环境变量覆盖语义必须一致。
- 新配置项需要合理默认值、示例配置、`WECHECKIN_*` 环境变量绑定和非法值校验，不把生产凭证写入默认文件。端口、超时、连接池和有依赖关系的配置在进程启动前统一校验。
- HTTP 主服务与 `maintenance` 对启动时数据库连接失败采用 fail-fast；`taskd` 对临时 MySQL/Redis 错误采用有上限的指数退避，`--role=all` 支持单节点同时运行 scheduler 和 worker 且不监听 HTTP。
- 数据库驱动必须配置连接/读/写超时和连接池边界。GORM 日志必须保持参数化 SQL；长驻轮询进程默认使用 `Warn` 级别。
- 外部 HTTP、钉钉 OpenAPI、Redis、通知和文件存储使用可替换边界，设置超时、最大请求/响应大小、重定向与目标网络约束，并在 HTTP 边界转换外部错误。
- 日志保留请求 ID、业务键和内部错误链，不记录密码、token、完整证件号或其他敏感数据；客户端响应不得包含 SQL、堆栈、本地路径或下游私有错误。
