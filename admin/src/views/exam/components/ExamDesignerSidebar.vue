  ...
  <div class="survey-sidebar-panel" :class="{ compact: middleTab === 'logic' }">
    ...
    <div v-show="middleTab !== 'logic' && middleTab !== 'result'" class="survey-sidebar-panel-tabs-content">
      <el-tabs v-if="middleTab === 'item'" v-model="sideSubTab" class="side-sub-tabs">
        ...
        <el-tab-pane label="大纲" name="outline"><SurveyDesignerOutline :title="form.title || '未命名考试'" :questions="questions" :selected-id="selectedId" :invalid-ids="[]" :type-name="typeName" @select="emit('select', $event)" /></el-tab-pane>
      </el-tabs>
        ...
        <el-tabs v-model="appearanceTab" class="appearance-tabs">
          <el-tab-pane v-for="resourceType in resourceTypes" :key="resourceType.name" :label="resourceType.label" :name="resourceType.name">
            <div class="appearance-grid">
              ...
              <el-upload action="/api/v2/admin/exam-resources" :show-file-list="false" :on-success="handleUploadSuccess" :on-error="handleUploadError" :headers="uploadHeaders" accept="image/*" :data="{ examId: form.id, resType: resourceType.name }" :before-upload="checkSaved"><div class="appearance-add">+</div></el-upload>
            </div>
...
import SurveyDesignerOutline from '../../survey/components/SurveyDesignerOutline.vue'
import SurveyDesignerQuestionPalette from '../../survey/components/SurveyDesignerQuestionPalette.vue'
import type { SurveyDesignerQuestion } from '../../survey/survey-designer-types'

const props = defineProps<{ form: Record<string, any>; types: any[]; questions: SurveyDesignerQuestion[]; selectedId: string | null; typeName: (type: string) => string }>()
...
function loadBank() {
  ...
  bankTimer = setTimeout(async () => {
    ...
    try {
      const response: any = await adminApi.examQuestionBankList({ keyword: bankKeyword.value, page: 1, pageSize: 100 })
...
async function removeResource(field: string, item: any) {
  if (item.id) { try { await adminApi.examResourceDelete({ id: item.id }) } catch {} }
  if (isActiveImage(field, item)) { form.value[field] = []; emit('save') }
  await loadResources()
}
function checkSaved() { if (!form.value.id) { ElMessage.warning('请先保存考试'); return false }; return true }
...
async function loadResources() {
  ...
  try {
    const response: any = await adminApi.examResourceList({ examId: form.value.id })
...
.survey-sidebar-panel.compact { width:48px; }
.survey-sidebar-panel.compact .survey-sidebar-panel-tabs-content { display:none; }
