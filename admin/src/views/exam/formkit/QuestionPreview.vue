  ...
  <div class="question-preview" :class="{ editing, 'is-hidden': q.defaultHidden }">
      ...
      <div style="overflow-x:auto">
        ...
        <template v-if="q.type==='matrixAuto' && (!q.props?.rows || !q.props.rows.length)">
          <div class="matrix-edit-row" :style="{display:'grid',gridTemplateColumns:matrixGridCols,gap:0,padding:0,cursor:'default',opacity:0.6}">
            ...
            <div v-for="col in (q.props?.columns||[])" :key="col.id||col.label||col" style="padding:4px;border:1px solid #e8e8e8;border-top:none;border-left:none;min-width:0"><el-input disabled size="small" :placeholder="col.title||col.label||'值'" style="pointer-events:none" /></div>
          </div>
        </template>
    ...
    <div v-if="!editing" class="preview-body">
      ...
      <div v-else-if="q.type==='matrixAuto'" class="preview-matrix">
        <table><thead><tr><th class="corner">#</th><th v-for="c in (q.props?.columns||[])" :key="c.id||(typeof c==='string'?c:c.title)">{{ typeof c==='string'?c:(c.title||c.label) }}</th></tr></thead><tbody><tr v-for="(r, ri) in (q.props?.rows||[])" :key="ri"><td class="matrix-label">{{ ri+1 }}</td><td v-for="(c, ci) in (q.props?.columns||[])" :key="c.id||(typeof c==='string'?c:c.title)"><el-input :model-value="val[ri]?.[ci]" @update:model-value="(v: any) => { if(!val[ri]) val[ri]={}; val[ri][ci]=v }" size="small" :placeholder="c.title||c.label||'值'" style="width:100%" /></td></tr></tbody></table>
