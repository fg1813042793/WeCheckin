  ...
  <div class="page">
    ...
    <el-card shadow="never" class="main-card">
      ...
      <el-table :data="list" v-loading="loading" stripe>
        ...
        <el-table-column label="试卷" width="160">
          <template #default="{row}">{{ paperTitle(row.paperId) }}</template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="250" fixed="right">
          <template #default="{row}">
            ...
            <el-button size="small" type="primary" @click="goRecords(row)">记录</el-button>
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.isCreate?'新建考试':'编辑考试'" width="700px">
      <el-form :model="form" label-width="110px">
        ...
        <el-form-item label="关联试卷">
          <el-select v-model="form.paperId" style="width:100%" filterable>
...
async function loadPapers() {
  try {
    const res: any = await adminApi.examPaperList({ page: 1, pageSize: 200 })
    papers.value = res.list || []
  } catch {}
}

function paperTitle(pid: number) {
  const p = papers.value.find((x: any) => x.id === pid)
...
const form = reactive<any>({ id: 0, title: '', paperId: null, startDate: null, endDate: null, duration: 60, maxAttempts: 1, showScoreBool: 1, statusBool: 1 })
...
function showAdd() {
  Object.assign(form, { id: 0, title: '', paperId: null, startDate: null, endDate: null, duration: 60, maxAttempts: 1, showScoreBool: 1, statusBool: 1 })
  dialog.isCreate = true; dialog.visible = true
}

function showEdit(row: any) {
  Object.assign(form, {
    id: row.id, title: row.title, paperId: row.paperId,
...
async function save() {
  ...
  try {
    const payload: any = {
      title: form.title, paperId: form.paperId,
    ...
    if (dialog.isCreate) { await adminApi.examInsert(payload); ElMessage.success('已创建') }
    else { payload.id = form.id; await adminApi.examEdit(payload); ElMessage.success('已更新') }
...
async function del(row: any) {
  await adminApi.examDel({ id: row.id })
  ElMessage.success('已删除'); load()
}

function goRecords(row: any) {
  router.push({ path: '/survey/exam/record', query: { examId: String(row.id), title: row.title } })
}
